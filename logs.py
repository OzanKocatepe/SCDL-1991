...
# Creates the required log config and starts logging.
def start_logging(scf, log_file):
    ...
    log_config.data_received_cb.add_callback(lambda _timestamp, data, _logconf: log_callback(scf.cf.link_uri, data, log_file))
    log_config.start()

def log_callback(uri, data, log_file):
    ...
    try:
        ...
        file.write("URI,x,y,z,vx,vy,vz,batV,bat%\n")
    # If the file already exists, we will simply append to it.
    except:
        file = open(log_file, "a")

    # Writes to the file in csv form and then closes it.
    file.write(f'{uri},{pos[0]},{pos[1]},{pos[2]},{velocity[0]},{velocity[1]},{velocity[2]},{data["pm.vbat"]},{data["pm.batteryLevel"]}\n')
    file.close()